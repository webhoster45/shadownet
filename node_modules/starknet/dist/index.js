...
__export(api_exports, {
  ...
  PAYMASTER_API: () => import_starknet_types_010.PAYMASTER_API,
  RPCSPEC010: () => RPCSPEC010,
  RPCSPEC09: () => RPCSPEC09
});
...
var RPCSPEC010 = __toESM(require("@starknet-io/starknet-types-010"));
var import_starknet_types_010 = require("@starknet-io/starknet-types-010");
...
// src/channel/rpc_0_10_0.ts
var rpc_0_10_0_exports = {};
...
var RpcChannel2 = class {
   ...
   * RPC specification version this Channel class implements
   */
  channelSpecVersion = _SupportedRpcVersion.v0_10_0;
  nodeUrl;
  headers;
  requestId;
  blockIdentifier;
  ...
  simulateTransaction(invocations, simulateTransactionOptions = {}) {
    ...
    const block_id = new Block(blockIdentifier).identifier;
    const simulationFlags = [];
    if (skipValidate) simulationFlags.push(RPCSPEC010.ESimulationFlag.SKIP_VALIDATE);
    if (skipFeeCharge) simulationFlags.push(RPCSPEC010.ESimulationFlag.SKIP_FEE_CHARGE);
    return this.fetchEndpoint("starknet_simulateTransactions", {
      block_id,
      transactions: invocations.map((it) => this.buildTransaction(it)),
      simulation_flags: simulationFlags
  ...
  async waitForTransaction(txHash, options) {
    ...
    const successStates = options?.successStates ?? [
      // RPC.ETransactionExecutionStatus.SUCCEEDED, // UDC  on SUCCEEDED + pre_confirmed had no proper events to parse UDC
      RPCSPEC010.ETransactionFinalityStatus.ACCEPTED_ON_L2,
      RPCSPEC010.ETransactionFinalityStatus.ACCEPTED_ON_L1
    ];
    const errorMessages = {
      [RPCSPEC010.ETransactionStatus.RECEIVED]: SYSTEM_MESSAGES.txEvictedFromMempool,
      [RPCSPEC010.ETransactionStatus.PRE_CONFIRMED]: SYSTEM_MESSAGES.consensusFailed,
      [RPCSPEC010.ETransactionStatus.CANDIDATE]: SYSTEM_MESSAGES.txFailsBlockBuildingValidation
    };
    const txLife = [];
    let txStatus;
    while (!onchain) {
  ...
  async fastWaitForTransaction(txHash, address, initNonceBN, options) {
    ...
    let retries = options?.retries ?? 50;
    const retryInterval = options?.retryInterval ?? 500;
    const errorStates = [RPCSPEC010.ETransactionExecutionStatus.REVERTED];
    const successStates = [
      RPCSPEC010.ETransactionFinalityStatus.ACCEPTED_ON_L2,
      RPCSPEC010.ETransactionFinalityStatus.ACCEPTED_ON_L1,
      RPCSPEC010.ETransactionFinalityStatus.PRE_CONFIRMED
    ];
    let txStatus;
    const start = (/* @__PURE__ */ new Date()).getTime();
    while (retries > 0) {
  ...
  buildTransaction(invocation, versionType) {
    ...
    if (invocation.type === api_exports.ETransactionType.INVOKE) {
      const btx = {
        type: RPCSPEC010.ETransactionType.INVOKE,
        sender_address: invocation.contractAddress,
        calldata: CallData.toHex(invocation.calldata),
        ...details
      };
...
var RpcProvider = class {
    ...
    if (optionsOrProvider && "channel" in optionsOrProvider) {
      ...
      if (options && options.specVersion) {
        if (isVersion("0.9", options.specVersion)) {
          ...
          this.channel = new rpc_0_10_0_exports.RpcChannel({ ...options, waitMode: false });
  ...
  static async create(optionsOrProvider) {
    ...
    if (isVersion("0.10", spec)) {
      return new this({
        ...optionsOrProvider,
        specVersion: _SupportedRpcVersion.v0_10_0
      });
    }
    throw new LibraryError(
  ...
  async getEvents(eventFilter) {
    if (this.channel instanceof rpc_0_10_0_exports.RpcChannel) {
